...
The **Toolbar Analysis** (below the board) displays suggestions from one source at a time. When multiple engines or saved results are available, an source selector appears.
...
- **Right-click** the source selector icon to toggle analysis visualizations on the board, or press <kbd>V</kbd>.
...
- "Auto-save Each Position" automatically saves results for each position as it is analyzed.
