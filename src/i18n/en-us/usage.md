...
- To save results to notes, use the menu in each engine's section.
  ...
  - "Save Evaluation Marks" writes evaluation marks (`!!`, `!`, `?`, `??`) into the PTN based on differences in evaluation scores between consecutive positions.
- To clear an engine's unsaved results, use the menu to select "Clear Results" or "Clear Current Position."
- To remove evaluation marks, use the delete menu and select "Remove Evaluation Marks." This strips `!!`, `!`, `?`, and `??` from the PTN while preserving tak/tinue marks.
...
The Engine Analysis and Saved Results settings both include options for managing results:

- "Suggestions to Save per Engine" limits how many suggestions are saved per position for each engine.
- "Auto-save after Search" automatically saves results after any finite search completes.
- "Overwrite Inferior Results" only replaces saved results if the new results have higher nodes or time values.
- "Show Evaluation Marks" toggles the display of dynamic evaluation marks on moves. This setting is available in both Engine Analysis and Saved Results settings.
- "Evaluation Mark Thresholds" configures the score-loss thresholds for Brilliant (`!!`), Good (`!`), Bad (`?`), and Blunder (`??`).
...
Dynamic evaluation marks are computed on-the-fly from whichever analysis source is active (unsaved engine results on the Engines tab, or saved results on the Saved tab). When "Show Evaluation Marks" is enabled, dynamic marks are merged with any marks already saved in the PTN, with dynamic marks taking precedence. Evaluation marks must be saved manually using "Save Evaluation Marks" in the engine's save menu.
